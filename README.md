...
A simple Streamlit dashboard for the 14-week "Road to New Year" lean bulk
(28 Sep 2026 to 3 Jan 2027). Log your gym sessions and morning weigh-ins, mark
whether you hit your macros, then watch your progress against the plan.
...
| Page | What it does |
|---|---|
| **Dashboard** (`Dashboard.py`) | Week you are in, today's gym session with sets, rep ranges and last top set, today's diet targets with the plan's +/- 200 kcal rule applied, bodyweight chart with 7-day average and the +0.25 kg/week target line, this week's checklist, new bests in the last 7 days. |
...
| **Log Bodyweight** | One weigh-in per morning, plus sleep hours and whether you hit yesterday's macros (hit / mostly / missed). Saving the same date again replaces it. |
...
| **Weekly Review** | One row per plan week with average weight, change, gym sessions, macro days and sleep; sets per week by session type; weekly weight change vs target. |
...
plan.py           The plan as data: dates, sessions, rep ranges, macros. Edit this if the plan changes.
